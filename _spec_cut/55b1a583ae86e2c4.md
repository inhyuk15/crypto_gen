### **ACORN: A Lightweight Authenticated Cipher (v1)** 
...
|**1**|**Specifcation**|**2**|
|---|---|---|
||1.1<br>Recommended parameter sets . . . . . . . . . . . . .|. . . . . . .<br>2|
||1.2<br>Operations, Variables and Functions . . . . . . . . .|. . . . . . .<br>2|
||1.2.1<br>Operations<br>. . . . . . . . . . . . . . . . . . .|. . . . . . .<br>2|
||1.2.2<br>Variables and constants . . . . . . . . . . . .|. . . . . . .<br>2|
||1.2.3<br>Functions . . . . . . . . . . . . . . . . . . . .|. . . . . . .<br>3|
||1.3<br>ACORN-128<br>. . . . . . . . . . . . . . . . . . . . . .|. . . . . . .<br>3|
...
||3.1<br>The security of the initialization<br>. . . . . . . . . . .|. . . . . . .<br>8|
||3.2<br>The security of the encryption process . . . . . . . .|. . . . . . .<br>9|
||3.3<br>The security of message authentication . . . . . . . .|. . . . . . .<br>9|
|**4**|**Features**|**11**|
...
_Si,_ 289 = _Si,_ 289 _⊕ Si,_ 235 _⊕ Si,_ 230; _Si,_ 230 = _Si,_ 230 _⊕ Si,_ 196 _⊕ Si,_ 193; _Si,_ 193 = _Si,_ 193 _⊕ Si,_ 160 _⊕ Si,_ 154; _Si,_ 154 = _Si,_ 154 _⊕ Si,_ 111 _⊕ Si,_ 107; _Si,_ 107 = _Si,_ 107 _⊕ Si,_ 66 _⊕ Si,_ 61; _Si,_ 61 = _Si,_ 61 _⊕ Si,_ 23 _⊕ Si,_ 0; _fi_ = _FBK_ 128( _Si, cai, cbi_ ) ; for _j_ := 0 to 291 do _Si_ +1 _,j_ = _Si,j_ +1 ; _Si_ +1 _,_ 292 = _fi ⊕ mi_ ; 

##### **1.3.3 The initialization of ACORN-128** 

The initialization of ACORN-128 consists of loading the key and _IV_ into the state, and running the cipher for 1536 steps. 

1. Initialize the state _S−_ 1536 to 0. 

2. Let _m−_ 1536+ _i_ = _K_ 128 _,i_ for _i_ = 0 to 127; Let _m−_ 1536+128+ _i_ = _IV_ 128 _,i_ for _i_ = 0 to 127; Let _m−_ 1536+256 = 1; Let _m−_ 1536+256+ _i_ = 0 for _i_ = 1 to 1279; 

4 

3. Let _ca−_ 1536+ _i_ = 1 for _i_ = 0 to 1535; Let _cb−_ 1536+ _i_ = 1 for _i_ = 0 to 1535; 

4. for _i_ = _−_ 1536 to _−_ 1, _Si_ +1 = StateUpdate128( _Si_ , _mi_ , _cai_ , _cbi_ ); 
...
1. Let _mi_ = _adi_ for _i_ = 0 to _adlen −_ 1; 

   - Let _madlen_ = 1; 

   - Let _madlen_ + _i_ = 0 for _i_ = 1 to 511; 

2. Let _cai_ = 1 for _i_ = 0 to adlen+255; Let _cai_ = 0 for _i_ = adlen+256 to adlen+511; Let _cbi_ = 1 for _i_ = 0 to adlen+511; 

3. for _i_ = 0 to _adlen_ + 511, _Si_ +1 = StateUpdate128( _Si_ , _mi_ , _cai_ , _cbi_ ); 

Note that even when there is no associated data, we still need to run the cipher for 512 steps. When we process the associated data, the keystream bit is used to update the state since _cbi_ = 1. The cipher specification is changed for 256 steps (since the value of _cai_ is set to 0 for 256 steps) so as to separate the associate data from the plaintext/ciphertext. 
...
1. Let _madlen_ +512+ _i_ = _pi_ for _i_ = 0 to _pclen −_ 1; Let _madlen_ +512+ _pclen_ = 1; Let _madlen_ +512+ _pclen_ + _i_ = 0 for _i_ = 1 to 511; 

2. Let _cai_ = 1 for _i_ = _adlen_ + 512 to _adlen_ + _pclen_ + 767; Let _cai_ = 0 for _i_ = _adlen_ + _pclen_ + 768 to _adlen_ + _pclen_ + 1023; Let _cbi_ = 0 for _i_ = _adlen_ + 512 to _adlen_ + _pclen_ + 1023; 

3. for _i_ = _adlen_ + 512 to _adlen_ + _pclen_ + 1023, 
...
Note that even when there is no plaintext, we still need to run the cipher for 512 steps. When we process the plaintext, the keystream bit is not used to update the state since _cbi_ = 0. The cipher specification is changed for 256 steps (since the value of _cai_ is set to 0 for 256 steps) so as to separate the processing of plaintext/ciphertext and the finalization. 
...
- After processing all the plaintext bits, we generate the authentication tag _T_ . 
   ...
   1. Let _madlen_ + _pclen_ +1024+ _i_ = 0 for _i_ = 0 to 511; 

   2. Let _cai_ = 1 for _i_ = _adlen_ + _pclen_ + 1024 to _adlen_ + _pclen_ + 1535; Let _cbi_ = 1 for _i_ = _adlen_ + _pclen_ + 1024 to _adlen_ + _pclen_ + 1535; 

   3. for _i_ = _adlen_ + _pclen_ + 1024 to _adlen_ + _pclen_ + 1535, 

_Si_ +1 = StateUpdate128( _Si_ , _mi_ , _cai_ , _cbi_ ); 

_ksi_ = _KSG_ 128( _Si_ ); end for; The authentication tag _T_ is the last _t_ keystream bits, i.e., _T_ = _ksadlen_ + _pclen_ +1535 _−t_ +1 _∥ ksadlen_ + _pclen_ +1535 _−t_ +2 _∥· · · ∥ ksadlen_ + _pclen_ +1535. **1.3.7 The decryption and verification** The decryption and verification are very similar to the encryption and tag generation. The finalization in the decryption process is the same as that in the encryption process. We emphasize that if the verification fails, the ciphertext and the newly generated authentication tag should not be given as output; 

otherwise, the state of ACORN-128 is vulnerable to known-plaintext or chosenciphertext attacks (using a fixed _IV_ ). 
...
The security goals of ACORN are given in Table 2.1. In ACORN, each key, IV pair is used to protect only one message. If verification fails, the new tag and the decrypted ciphertext should not be given as output. Note that the authentication security in Table 2.1 includes the integrity security of plaintext, associated data and nonce. 

Table 2.1: Security Goals of ACORN-128 <u>(128-bit</u> tag) Encryption Authentication ACORN-128 128-bit 128-bit 
...
The initialization can be attacked by analyzing the relation between IV and keystream. In ACORN-128, the IV passes through at least 1792 steps before 

8 

affecting ciphertext. This large number of steps in the initialization is to prevent the various attacks against stream cipher initialization: the linear attack (such as the attack in [13]), differential attacks (such as the attacks in [15] and [14]) and cube attacks [6, 7]. 
